...
<figure><img src="../../../../../.gitbook/assets/image (305).png" alt=""><figcaption><p>Figura #1: Captura de pantalla creación de Bono No Depósito.</p></figcaption></figure>
...
{% hint style="warning" %}
**Nota**: Los campos de configuración para el Rollover son dimámicos, se recomienda validar la sección de [validaciones y reglas del negocio](https://virtualsoft.gitbook.io/manuales/torneos-y-bonos/torneos-y-bonos./como-acceder-a-la-plataforma-./crear-eventos./crear-bono./bono-no-deposito.#id-6.-validaciones-y-reglas-de-negocio).
{% endhint %}
...
<table><thead><tr><th width="139.5">Campo</th><th width="119">Tipo de control</th><th>Descripción</th></tr></thead><tbody><tr><td><strong><code>Factor rollover de Bono</code></strong></td><td>Numérico</td><td><p>Número por el cual se multiplica el valor del bono para determinar el monto total que el usuario debe apostar antes de liberarlo.<br><strong>Fórmula:</strong><br><em>Rollover = Valor del bono × Factor rollover de Bono.</em></p><div data-gb-custom-block data-tag="hint" data-style="info" class="hint hint-info"><p><strong>Ejemplo:</strong> Con un bono de <strong>$10</strong> y un factor de rollover de <strong>5</strong>, el rollover es de <strong>$50</strong>. El usuario debe apostar $50 en total para liberar el bono; hasta alcanzar ese monto, el saldo bono no se convierte en saldo real.</p></div></td></tr><tr><td><strong><code>Monto de Bono Fijo</code></strong></td><td>Numérico</td><td>Valor fijo del bono que recibe el usuario.</td></tr><tr><td><strong><code>Máxima apuesta tomada para rollover</code></strong></td><td>Numérico</td><td><p>Monto máximo de una apuesta que se contabiliza para el rollover. Si el usuario apuesta un valor superior, el excedente no descuenta del rollover pendiente.</p><div data-gb-custom-block data-tag="hint" data-style="info" class="hint hint-info"><p><strong>Ejemplo:</strong> Con una máxima apuesta de <strong>$20</strong>, si el usuario realiza una apuesta de <strong>$50</strong>, solo se descuentan <strong>$20</strong> del rollover pendiente.</p></div></td></tr><tr><td><strong><code>Monto Rollover</code></strong></td><td>Numérico</td><td><p>Valor total que tendrá el Rollover.</p><div data-gb-custom-block data-tag="hint" data-style="warning" class="hint hint-warning"><p><strong>Nota</strong>: En caso de configurar este campo, se tomará este valor como único del Rollover y es ocultarán los campos adicionales.</p></div></td></tr><tr><td><strong><code>Jugadores</code></strong></td><td>Botón</td><td><p>Permite cargar un archivo CSV con los ID de los usuarios que reciben el bono.</p><div data-gb-custom-block data-tag="hint" data-style="warning" class="hint hint-warning"><p><strong>Nota:</strong> Este campo se utiliza cuando el bono se entrega mediante asignación directa. Los usuarios incluidos en el archivo reciben el bono sin necesidad de redimir un código.</p></div></td></tr></tbody></table>
...
* Los campos del rollover son dinámicos, según el tipo de configuración, desaparecerán los campos restantes.

{% hint style="info" %}
**Ejemplo**: Si se selecciona "**Dar efectivo**" en el campo **`Asignación de bono`** los campos disponibles para configurar el rollover serán:

* Factor de rollover bono.
* Monto de bono fijo.
* Monto rollover.
* Máxima apuesta tomada para rollover.

Si se seleccionan varios bonos y se **excluye** "**Dar efectivo**"  los campos disponibles para la configuración serán:

* Monto Rollover.
* Máxima apuesta tomada para rollover.

Si se seleccionan uno o varios bonos y adicional se selecciona "**Dar efectivo**"  los campos disponibles para la configuración serán:
